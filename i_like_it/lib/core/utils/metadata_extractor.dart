...
class MetadataExtractor {
    ...
    try {
      ...
      if (urlWithProtocol.contains('youtube.com/watch') || 
        ...
        if (urlWithProtocol.contains('youtu.be/')) {
          ...
          videoId = urlWithProtocol.split('youtube.com/shorts/').last.split('?').first;
        } else {
          try {
        ...
        if (videoId != null && videoId.isNotEmpty) {
          ...
          try {
            final oembedResponse = await http.get(
              Uri.parse('https://noembed.com/embed?url=https://www.youtube.com/watch?v=$videoId')
  ...
  static bool _isLikelyId(String s) {
    ...
    if (s.length >= 5 && s.length <= 15 && RegExp(r'^(?=.*[a-zA-Z])(?=.*\d)[a-zA-Z\d_]+$').hasMatch(s)) return true;
    // Instagram / Facebook style post IDs
    if (s.startsWith('C_') || s.startsWith('p_') || s.startsWith('v_')) return true;
  ...
  static String _getFallbackTitle(String url) {
    try {
      ...
      if (host.contains('youtube.com') || host.contains('youtu.be')) {
        ...
        platform = 'Instagram';
      } else if (host.contains('facebook.com') || host.contains('fb.watch') || host.contains('fb.me')) {
        platform = 'Facebook';
      } else if (host.contains('tiktok.com')) {
        platform = 'TikTok';
      } else if (host.contains('twitter.com') || host.contains('t.co') || host.contains('x.com')) {
        platform = 'Twitter';
      } else {
      ...
      if (pathSegments.isNotEmpty) {
        ...
        final meaningfulSegments = pathSegments.where((s) {
          final isGeneric = s == 'p' || s == 'pin' || s == 'watch' || s == 'share' || s == 'posts' || s == 'post';
