...
class FolderClassificationService {
  static final FolderClassificationService instance = FolderClassificationService._();
  ...
  Future<String?> _ensureModelInitialized() async {
        ...
        if (response.statusCode == 200) {
            ...
            for (final pref in preferences) {
                final found = models.firstWhere(
                    (m) => m['name'] == pref && (m['supportedGenerationMethods'] as List).contains('generateContent'),
                    orElse: () => {},
                );
                
                if (found.isNotEmpty) {
                    print('AI Service: Selected model ${found['name']}');
                    _validatedModelEndpoint = 'https://generativelanguage.googleapis.com/v1beta/${found['name']}:generateContent';
                    return _validatedModelEndpoint;
                }
            }
            ...
            final anyModel = models.firstWhere(
                (m) => (m['name'] as String).contains('gemini') && (m['supportedGenerationMethods'] as List).contains('generateContent'),
                 orElse: () => {},
            );
            
             if (anyModel.isNotEmpty) {
                print('AI Service: Fallback model ${anyModel['name']}');
                _validatedModelEndpoint = 'https://generativelanguage.googleapis.com/v1beta/${anyModel['name']}:generateContent';
                return _validatedModelEndpoint;
            }
        } else {
            print('AI Service: Failed to list models. HTTP ${response.statusCode}: ${response.body}');
        }
    } catch (e) {
    ...
    _validatedModelEndpoint = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent';
    return _validatedModelEndpoint;
  }
  ...
  Future<void> classifyFolder(dynamic folderId, String folderName) async {
    ...
    try {
      ...
      if (category != null) {
        // Update Local DB
        await DatabaseHelper.instance.updateFolderSystemCategory(folderId, category);
        print('FolderClassificationService: Updated local folder $folderId with "$category"');
        
        // Trigger Sync to push to Cloud
        SyncManager.instance.sync();
      }
    } catch (e) {
      print('FolderClassificationService: Failed to classify "$folderName": $e');
    }
  }
  ...
  Future<int> runBatchMigration() async {
    ...
    final uncategorized = await SyncManager.instance.remoteDataSource.fetchFoldersWithoutSystemCategory();
    ...
    for (final batch in batches) {
      ...
      if (updates.isNotEmpty) {
        await SyncManager.instance.remoteDataSource.batchUpdateSystemCategories(updates);
  ...
  Future<String?> _getCategoryWithRetry(String folderName, {int retries = 2}) async {
    for (int i = 0; i <= retries; i++) {
      try {
        return await _callGemini(folderName);
      } catch (e) {
        if (i == retries) {
          print('Gemini HTTP failed "$folderName" after $retries retries. Error: $e');
          return 'other'; 
        }
...
''';
    ...
    final data = jsonDecode(response.body);
    final text = data['candidates']?[0]?['content']?['parts']?[0]?['text']?.toString().trim().toLowerCase() ?? '';
    
    // Clean up
    final cleanText = text.replaceAll(RegExp(r'[^a-z]'), '');

    print('AI Debug (HTTP): Input="$folderName" | Raw="$text" | Clean="$cleanText"');
