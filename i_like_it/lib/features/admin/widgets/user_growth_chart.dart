...
enum TimeRange { last7Days, last30Days, last6Months, lastYear, allTime }
...
class _UserGrowthChartState extends State<UserGrowthChart> {
  ...
  Future<void> _loadData() async {
    try {
      final dates = await SyncManager.instance.remoteDataSource
          .fetchUserGrowthData();
      if (mounted) {
        _allDates = dates;
        _processData();
  ...
  void _processData() {
    ...
    switch (_selectedRange) {
      ...
      case TimeRange.last6Months:
        filteredDates = _allDates
            .where((d) => _monthDifference(d, now) < 6)
            .toList();
        pointsCount = 6;
        groupByMonth = true;
        break;
      case TimeRange.lastYear:
        filteredDates = _allDates
            .where((d) => _monthDifference(d, now) < 12)
            .toList();
        pointsCount = 12;
        groupByMonth = true;
        break;
    ...
    if (!groupByMonth) {
      ...
      if (_selectedRange == TimeRange.last6Months) {
        maxMonths = 5;
      } else if (_selectedRange == TimeRange.lastYear)
        maxMonths = 11;
      else {
        // All time: find oldest
        if (_allDates.isNotEmpty) {
          maxMonths = _monthDifference(
            _allDates.first,
            now,
          ); // e.g. 24 months ago
        }
    ...
    for (int i = 0; i < pointsCount; i++) {
      ...
      if (!groupByMonth) {
        ...
        if (_selectedRange == TimeRange.last6Months) {
          ...
          maxMonths = 11;
        else if (_allDates.isNotEmpty)
          maxMonths = _monthDifference(_allDates.first, now);
                ...
                titlesData: FlTitlesData(
                  ...
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      showTitles: true,
                      reservedSize: 30,
                      interval: _titles.length > 12
                          ? (_titles.length / 6).ceilToDouble()
                          : 1, // Skip labels if too many
